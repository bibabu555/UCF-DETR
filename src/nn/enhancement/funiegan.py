...
class GeneratorFunieGAN(nn.Module):
    ...
    """
    __inject__ = []
    def __init__(self, in_channels=3, out_channels=3, pretrained=None):
        ...
        if pretrained:
            print(f"Loading pretrained weights for enhancer from: {pretrained}")
            try:
                # Use map_location to ensure it works on both CPU and GPU
                self.load_state_dict(torch.load(pretrained, weights_only=True))
                print("Enhancer weights loaded successfully.")
            except Exception as e:
                print(f"Error loading enhancer weights: {e}")
    def forward(self, x):
        d1 = self.down1(x)
        d2 = self.down2(d1)
        d3 = self.down3(d2)
