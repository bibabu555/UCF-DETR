...
class QueryInteraction(nn.Module):
    def __init__(self, d_model, n_head, dropout=0., adapter_ratio=4):
        super().__init__()
        
        # Feature Adapter: Map the reference features to the target domain
        # 使用 MLP 增加非线性能力，处理水下图像和增强图像的复杂域差异
    ...
    def _init_weights(self):
        ...
        init.constant_(self.gate_net[-2].bias, -4.0)
        
    def forward(self, tgt, ref, pos_tgt, pos_ref, attn_mask=None):

        # Adapt reference features
        ref_adapted = self.adapter(ref)
        
        # Cross Attention: Query=tgt, Key/Value=ref
        # Apply dropout once to the attention output
        tgt2, _ = self.attn(
            query=tgt + pos_tgt,
            key=ref_adapted + pos_ref,
            value=ref_adapted,
            attn_mask=attn_mask
        )
        tgt2 = self.dropout(tgt2)
        
        # Dynamic Gating: Compute based on [Original, Update]
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        # Query Interaction
        self.query_interaction = QueryInteraction(d_model, n_head, dropout=dropout)
        self.query_interaction2 = QueryInteraction(d_model, n_head, dropout=dropout)
    ...
    def forward(self,
        ...
        target2, target_enhance = target.split(bs//2, dim=0)
        pos, pos_enhance = query_pos_embed.split(bs//2, dim=0)
        target3 = self.query_interaction(target2, target_enhance, pos, pos_enhance, attn_mask)
        target4 = self.query_interaction2(target_enhance, target2, pos_enhance, pos, attn_mask)
        target = torch.cat([target3, target4], dim=0)
...
class RTDETRTransformerv2(nn.Module):
    ...
    def forward(self, feats, enhanced_feats, targets=None):
        bs = feats[0].shape[0]  # Store original batch size
        feats = [torch.cat([f, ef], dim=0) for f, ef in zip(feats, enhanced_feats)] 
        # input projection and embedding
        memory, spatial_shapes = self._get_encoder_input(feats) # [2N, C, H, W]
        
        targets_extended = None
        if targets is not None:
            targets_extended = targets + copy.deepcopy(targets)
        
        # prepare denoising training
        if self.training and self.num_denoising > 0:
            denoising_logits, denoising_bbox_unact, attn_mask, dn_meta = \
                get_contrastive_denoising_training_group(targets_extended, \
                    self.num_classes, 
                    self.num_queries, 
        ...
        out_logits_orig, out_logits_enh = out_logits.split(bs, dim=1)
        out_bboxes_orig, out_bboxes_enh = out_bboxes.split(bs, dim=1)
        
        out = {'pred_logits': out_logits_orig[-1], 'pred_boxes': out_bboxes_orig[-1]}

        if self.training and self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(out_logits_orig[:-1], out_bboxes_orig[:-1])
            
            if self.use_enhanced_loss:
                out['enhanced_aux_outputs'] = self._set_aux_loss(out_logits_enh, out_bboxes_enh)
                
            enc_logits_orig = [x.split(bs, dim=0)[0] for x in enc_topk_logits_list]
            enc_bboxes_orig = [x.split(bs, dim=0)[0] for x in enc_topk_bboxes_list]
            
            out['enc_aux_outputs'] = self._set_aux_loss(enc_logits_orig, enc_bboxes_orig)
            out['enc_meta'] = {'class_agnostic': self.query_select_method == 'agnostic'}

            if self.use_enhanced_loss:
                enc_logits_enh = [x.split(bs, dim=0)[1] for x in enc_topk_logits_list]
                enc_bboxes_enh = [x.split(bs, dim=0)[1] for x in enc_topk_bboxes_list]
                out['enc_enhanced_aux_outputs'] = self._set_aux_loss(enc_logits_enh, enc_bboxes_enh)
            
            if dn_meta is not None:
                dn_out_logits_orig, dn_out_logits_enh = dn_out_logits.split(bs, dim=1)
                dn_out_bboxes_orig, dn_out_bboxes_enh = dn_out_bboxes.split(bs, dim=1)
                
                dn_meta_orig = {
                    'dn_positive_idx': dn_meta['dn_positive_idx'][:bs],
                    'dn_num_group': dn_meta['dn_num_group'],
                    'dn_num_split': dn_meta['dn_num_split']
                }
                
                out['dn_aux_outputs'] = self._set_aux_loss(dn_out_logits_orig, dn_out_bboxes_orig)
                out['dn_meta'] = dn_meta_orig
                ...
                if self.use_enhanced_loss:                    
                    ...
                    out['dn_enhanced_aux_outputs'] = self._set_aux_loss(dn_out_logits_enh, dn_out_bboxes_enh)
                    out['dn_meta_enh'] = dn_meta_enh
